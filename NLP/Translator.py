...
def translate_file(file_path, dest_lang):
    if file_path.endswith('.xlsx'):
        df = pd.read_excel(file_path)
        df = df.applymap(lambda x: translate_text(str(x), dest_lang))
...
def main():
    ...
    if input_type == "Text":
        ...
        if st.button("Translate"):
            dest_lang = st.selectbox("Select output language:", ["en", "fr", "es"])  # Add more languages as needed
            translated_text = translate_text(text, dest_lang)
        ...
        if file is not None:
            ...
            dest_lang = st.selectbox("Select output language:", ["en", "fr", "es"])  # Add more languages as needed
            translate_file(file_path, dest_lang)
